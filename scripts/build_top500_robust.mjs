...
async function run() {
    ...
    for (let i = 0; i < players.length; i += CONCURRENCY) {
        ...
        const promises = batch.map(async (name) => {
            try {
                ...
                if (!data) { console.log('No data for ', name); return; } if (data.error) { console.log('Error for ', name, data.error); return; }
    ...
    fs.writeFileSync(LEADERBOARD_FILE, JSON.stringify(leaderboard, null, 2)); console.log('Leaderboard updated with ', leaderboard.length);
